# Learning Guide: Controller Test Automation Framework
...
1. `controller_framework/virtual_controller.py`
   - Learn the controller states, sensor validation, command handling, and fault latching.
2. `test_cases/controller_test_cases.json`
   - Learn how each system-level test is represented as steps with expected results.
3. `controller_framework/test_runner.py`
   - Learn how the runner loads test cases, executes steps, validates expectations, and writes logs.
4. `controller_framework/log_analyzer.py`
   - Learn how failures are grouped into useful debugging summaries.
5. `tests/test_virtual_controller.py`
   - Learn the smaller unit tests that protect the controller behavior.
...
I built a Python framework that validates a simulated controller using data-driven system tests. The controller has realistic states like OFF, IDLE, ACTIVE, and FAULT. The test runner executes JSON-defined test cases repeatedly, checks expected states and outputs, writes structured logs, and includes a failure analyzer that groups recurring failures by signature. This mirrors how system automation tests are maintained and triaged in embedded-controller validation.

## Practice Changes

To make the project more yours, try these changes:
